...
def get_article_details(url):
    ...
    # A. Extract the article title (often in <h1>)
    title_tag = soup.find("h1")
    title = title_tag.get_text(strip=True) if title_tag else ""
    
    # B. Extract the article summary
    #    Sometimes found in <p class="short_intro"> or similar
    summary_tag = soup.find("p", class_="short_intro")
    summary = summary_tag.get_text(strip=True) if summary_tag else ""
    
    # C. Extract the main content
    #    VnExpress sometimes uses <article> or a <div class="fck_detail"> for content
    content_div = soup.find("article") or soup.find("div", class_="fck_detail")
    if content_div:
...
if __name__ == "__main__":
    ...
    # 4. Write the results to a CSV file with utf-8-sig encoding
    csv_file = "vnexpress_news_1.csv"
    with open(csv_file, mode="w", newline="", encoding="utf-8-sig") as f:
        writer = csv.DictWriter(f, fieldnames=["No", "Title", "Summary", "Content", "Link"])
        writer.writeheader()
        for art in articles:
            writer.writerow(art)
