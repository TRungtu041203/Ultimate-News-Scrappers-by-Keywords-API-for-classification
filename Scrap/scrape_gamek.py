...
def get_search_results_page(keyword, page=1):
    ...
    search_url = f"https://gamek.vn/tim-kiem.chn?keyword={keyword}&page={page}"
    try:
    ...
    articles = soup.find_all("div", class_="f1 w445")  # Updated class selector
    ...
    for art in articles:
        ...
        if link and not link.startswith("http"):
            link = f"https://gamek.vn/{link}"  # Append base URL if needed
        
        # Extract summary from a <p> tag
...
def get_article_details(url):
    ...
    Given a link to a motgame.vn article, fetch and return:
      - Title (from <h1 data-field="title" data-role="title">)
      - Date (from <p class="mgt15">)
    ...
    # Extract title from <h1 data-field="title" data-role="title">
    title_tag = soup.find("h1", {"data-field": "title", "data-role": "title"})
    title_detail = title_tag.get_text(strip=True) if title_tag else ""
    
    # Extract summary from <div class="motgame-detail-desc">
    summary_tag = soup.find("h2", {"data-field": "sapo", "data-role": "sapo"})
    ...
    content_div = soup.find("div", class_="rightdetail_content detailsmallcontent")
    content = content_div.get_text(separator="\n", strip=True) if content_div else ""
    
    # Extract date from <p class="mgt15">
    date_tag = soup.find("p", class_="mgt15")
    date = ""
    if date_tag:
        # Extract the text content and split to isolate the date
        date_text = date_tag.get_text(strip=True)
        if "|" in date_text:
            date = date_text.split("|")[-1].strip()  # Get the part after the "|"
...
if __name__ == "__main__":
    ...
    csv_file = os.path.join(output_dir, f"gamek_{timestamp}.csv")
